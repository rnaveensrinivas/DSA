...
If the front of the queue is at the end of the list:

- **Enqueue** (`append` to the end): $O(n)$, since all elements must shift one position to the right.
- **Dequeue** (`pop` from the front): $O(1)$, just remove from last position.
